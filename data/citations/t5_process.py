    ...
    for i in range(len(splits)):
        ...
        if len(token) > 2 and token[0] == "<":
            ...
            if token[1] == "/":
                ...
                stack.pop()
                if len(stack) == 0:
                    assert not new_tag, f"{entity} ended with a tag!"
                    if len(entity):
    ...
    for i in range(len(splits)):
        ...
        if len(token) > 2 and token[0] == "<":
            ...
            if token[1] == "/":
                ...
                tag = token[1:-1]
                stack.append(tag)
                entity = entity + (" <entity_sep> " if len(entity) else "") + tag
                closed = False
                new_open = True
                high_level = check_level(splits, i)
        elif len(token) > 0:
            ...
            if len(entity) == 0 or len(stack) == 0 or closed or high_level:
                continue
            entity += (" <entity_text> " if new_open else "") + token
            new_open = False
    return " | ".join(entities), " ".join(sentence)
